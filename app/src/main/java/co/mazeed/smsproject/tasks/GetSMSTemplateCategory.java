...
/**
 * Created by neolptp on 7/6/2015.
 */
public class GetSMSTemplateCategory extends Task {
    ...
    public HashMap<Integer,TemplateCategory> templatesCategoryList= new HashMap<>();

    public GetSMSTemplateCategory(DataRequestor requestor, Context context, HashMap<String, String> inputs) {
        setRequestor(requestor);
        setId(TaskID.GetTemplatesCategoryTask);
        this.mxontext = context;
        url = Communication.Common_Base_URL + "getReadymadeSMSCategoryList/?";
        ...
        if (response.getStatusCode() == 200) {
            try {
                ...
                if(status.equalsIgnoreCase("Success"))
                {
                    ...
                    JSONArray smsTemplateCategoryList=mainObject.getJSONArray("smsTemplateCategoryList")	;
                    for (int i = 0; i < smsTemplateCategoryList.length(); i++) {
                        JSONObject group=smsTemplateCategoryList.getJSONObject(i);
                        TemplateCategory data=TemplateCategory.fromJson(group.toString());
                        templatesCategoryList.put(data.getCategoryID(),data);
                    }
                    ServiceStorage.templatesCategoryList=templatesCategoryList;
