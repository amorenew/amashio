...
//addContactGroup
public class AddContactGroup  extends Task {
	...
	public AddContactGroup(DataRequestor requestor, Context context, HashMap<String, String>inputs) {
		setRequestor(requestor);
		setId(TaskID.AddContactGroupTask);
		this.mxontext = context;
		url = Communication.Common_Base_URL + "addContactGroup/";
		...
		if (response.getStatusCode() == 200) {
			try {
				...
				if(status.equalsIgnoreCase("Success"))
				{
					...
					String tokenkey=mainObject.getString("groupID");
