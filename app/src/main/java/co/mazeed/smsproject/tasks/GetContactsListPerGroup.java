 ...
 */
public class GetContactsListPerGroup extends Task

    {
        ...
        public GetContactsListPerGroup(DataRequestor requestor, Context context, HashMap<String, String> inputs) {
        setRequestor(requestor);
        setId(Task.TaskID.GetContactsPerGroupTask);
        this.mxontext = context;
        url = Communication.Common_Base_URL + "getContactsList/?";
        ...
        if (response.getStatusCode() == 200) {
            try {
                ...
                if(status.equalsIgnoreCase("Success"))
                {
                    ...
                    JSONArray contactslist=mainObject.getJSONArray("contactList")	;
                    for (int i = 0; i < contactslist.length(); i++) {
                        JSONObject conatct=contactslist.getJSONObject(i);
                        ContactData groupData= ContactData.fromJson(conatct.toString());
                        contactsList.add(groupData);
                    }
