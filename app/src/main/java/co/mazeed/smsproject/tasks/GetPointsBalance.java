...
public class GetPointsBalance  extends Task {
	...
	public GetPointsBalance(DataRequestor requestor, Context context, HashMap<String, String>inputs) {
		setRequestor(requestor);
		setId(TaskID.GetPointsBalanceTask);
		this.mxontext = context;
		url = Communication.Common_Base_URL + "getPointsBalance/?";
		...
		if (response.getStatusCode() == 200) {
			try {
				...
				if(status.equalsIgnoreCase("Success"))
				{
					...
					String tokenkey=mainObject.getString("pointsBalance");
					ServiceStorage.pointsBalance=tokenkey;
